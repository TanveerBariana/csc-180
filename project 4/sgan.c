...
#define PMUT            0.01  // probability of flipping each bit
...
int    decode2(int index);
void   getpreviousbest();
double evaluate(int value, int value2);
...
int main()
{
  ...
  do
  {
    ...
    for (i=0; i<POPULATION_SIZE; i++)
    {
      ...
      pool[i].fitness = evaluate(pool[i].value, pool[i].value2);
    }
...
void elite()
{
  if ((MAXMIN*beststring.fitness) > (MAXMIN*evaluate(decode(0), decode2(0))))
  {
...
{
  int value2 = 0;
  for (int i=19; i<CHROM_LENGTH; i++)
    value2 += (int) pow(2.0,(double)i) * pool[index].string[CHROM_LENGTH-1-i];
  return (value2);
}
...
double convRange(int raw)
{
  double outval = ((((double)raw)/1048575.0)*20.0)-10.0;
  return(outval);
}
